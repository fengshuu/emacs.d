...
(use-package 
  ...
  (defmacro chunyang-use-package-keywords-add (keyword) 
    ...
    `(progn (add-to-list 'use-package-keywords ,keyword 'append) 
...
(use-package 
  chunyang-scratch 
  :preface (defun chunyang-scratch-save () 
             (ignore-errors (with-current-buffer "*scratch*" (write-region nil nil
                                                                           "~/Dropbox/feng/scratch")))) 
  (defun chunyang-scratch-restore () 
    (let ((f "~/Dropbox/feng/scratch")) 
      (when (file-exists-p f) 
        (with-current-buffer "*scratch*" (insert-file-contents f) 
                             (set-buffer-modified-p nil))))) 
  :init (add-hook 'kill-emacs-hook #'chunyang-scratch-save) 
...
(use-package 
  ...
  :no-require t 
  :defer t 
  :init (setq mac-command-modifier 'meta mac-option-modifier 'control))
...
(when *is-mac* (condition-case err (let ((path (with-temp-buffer (insert-file-contents-literally
                                                                  "~/.path") 
                                                                 (buffer-string)))) 
                                     (setenv "PATH" path) 
...
(use-package 
  chunyang-mac 
  :if *is-mac* 
  :commands (chunyang-mac-Terminal-send-region chunyang-mac-Terminal-cd
                                               chunyang-mac-iTerm-send-region
                                               chunyang-mac-iTerm-send-string chunyang-mac-iTerm-cd
                                               chunyang-mac-Finder-reveal
                                               chunyang-mac-edit-file-tags chunyang-mac-search-tags
                                               helm-chunyang-mac-tags chunyang-chrome-refresh
                                               chunyang-chrome-url chunyang-mac-app-running-p
...
(defun chunyang-frame-transparency-adjust () 
  ...
  (while (pcase (read-key (format "%2d%%  Press <up> and <down> to adjust" (frame-parameter nil
                                                                                            'alpha))) 
           ((or 
             (and 'up 
            ...
            (when (<= 0 new-alpha 100) 
              (setf (frame-parameter nil 'alpha) new-alpha)) t))))
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun chunyang-frame-tabs-filter (buffer _frame) 
             (let ((name (buffer-name buffer))) 
               (unless (or (eq (aref name 0) ?\s) 
                           (string-match-p "\\`\\*helm" name)) name))) 
  :config (setq frame-tabs-filter-function #'chunyang-frame-tabs-filter))
...
(use-package 
  ...
  :config (setq savehist-additional-variables '(extended-command-history Info-history-list
                                                                         ivy-views)) 
  (savehist-mode))

(use-package 
  recentf                               ; Recent files
  :preface (defun chunyang-recentf-save-list-for-alfred () 
...
See URL `https://www.alfredapp.com/help/workflows/inputs/script-filter/json/'." 
             ...
             (when (and recentf-mode 
                        recentf-list) 
               (write-region (json-encode-list `((items ,@(mapcar 
                                                           (lambda (f) 
                                                             (let ((abbrev (abbreviate-file-name f)) 
                                                                   (onlyname (file-name-nondirectory
                                                                              f))) 
                                                               `((uid . ,abbrev) 
                                                                 (type . "file") 
                                                                 (title . ,onlyname) 
                                                                 ...
                                                                 (icon . ((type . "fileicon") 
                                                                          (path . ,abbrev))))))
                                                           recentf-list)))) nil
                                                           "~/.recentf-alfred.json" nil 'silent))) 
  :config (setq recentf-max-saved-items 512 recentf-exclude '("/\\.git/.*\\'" ; Git contents
                                                              "/\\.emacs\\.d/elpa" ; ELPA
                                                              "/\\.emacs\\.d/etc/"
                                                              "/\\.emacs\\.d/var/"
                                                              "-autoloads\\.el\\'" "\\.elc\\'"
...
(use-package 
                    ...
                    (lambda (prompt) 
                      (let ((inhibit-read-only t) 
                        ...
                        (set-text-properties (point-min) 
                                             (point-max) props))))) 
               (pcase (list this-command (minibuffer-prompt) 
  ...
  (define-minor-mode prompt-watcher-mode "Watch the minibuffer prompt and customize if asking." 
    :global t 
    (if prompt-watcher-mode (add-hook 'minibuffer-setup-hook #'prompt-watcher) 
...
(use-package 
  chunyang-edit-minibuffer 
  :bind (:map minibuffer-local-map
              ("C-c '" . chunyang-edit-minibuffer)))
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :init (setq helm-advice-push-mark nil) 
  ...
  (define-minor-mode chunyang-helm-window-hack-mode "Hack helm window display." 
    :global t
    (let ((action '("\\`\\*helm" (display-buffer-in-side-window) 
                    (window-height . 0.4)))) 
      (if chunyang-helm-window-hack-mode (progn (add-to-list 'display-buffer-alist action) 
                                                (setq helm-display-function #'display-buffer)) 
        (setq display-buffer-alist (delete action display-buffer-alist)) 
...
(use-package 
  uniquify                       ; Make buffer names unique (turn on by default)
  :defer t 
            ...
            (interactive 
             (lambda (spec) 
               (if (and (boundp 'display-line-numbers) ; `display-line-numbers' was added in Emacs 26.1
                   ...
                   (unwind-protect (progn (display-line-numbers-mode) 
...
(use-package 
  chunyang-buffers 
  :preface (defun chunyang-kill-all-buffers 
               (&optional 
                except-current-buffer) 
             (interactive "P") 
             (let ((buffers-to-kill (seq-filter 
                                     (lambda (buffer) 
                                       (let ((name (buffer-name buffer)))
                                         ;; Ignore uninteresting buffers
                                         (and (not (string-prefix-p " " name)) 
                                              (not (member name '("*scratch*" "*Messages*"))) 
                                              (not (and except-current-buffer 
  ...
  (defun chunyang-kill-invisible-buffers () 
    ...
    (interactive) 
    (seq-do #'kill-buffer (seq-difference (seq-filter 
                                           (lambda (buffer) 
...
(use-package 
  ...
  :config (define-ibuffer-column human-readable-size 
            (:name "Size" 
                   :inline t 
                   :header-mouse-map ibuffer-size-header-map 
                   :summarizer (lambda (column-strings) 
                                 (cl-loop for s in column-strings sum (get-text-property (1- (length
                                                                                              s))
                                                                                         'size s)
                                          into total finally return (file-size-human-readable
                                                                     total)))) 
            (let ((size (buffer-size))) 
              (propertize (file-size-human-readable size) 'size size))) 
  (setq ibuffer-formats '((mark " " (name 18 18
                                          :left 
                                          :elide) " " (human-readable-size 6 -1 
                                                                           :right) " " (mode 16 16
                                                                                             :left 
                                                                                             :elide)
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun chunyang-ace-window (arg) 
...
One C-u, swap window, two C-u, `chunyang-window-click-swap'." 
             (interactive "p") 
             (cl-case arg (0 
                           (setq aw-ignore-on (not aw-ignore-on)) 
                           (ace-select-window)) 
                      (4 (ace-swap-window)) 
...
(use-package 
  ...
  :config (bind-keys :map winner-mode-map ("M-N" . winner-redo) 
                     ("M-P" . winner-undo)) 
  (winner-mode))
...
(use-package 
  ...
  :bind (("C-c f u" . revert-buffer) 
         ("C-c f n" . normal-mode)) 
  :preface (defun chunyang-add-file-local-variable-post 
               (&rest 
                _r)
             ...
             (when (y-or-n-p "Save and revert this file?") 
               ...
               (revert-buffer nil 
                              :no-confirm))) 
  (advice-add 'add-file-local-variable 
              :after #'chunyang-add-file-local-variable-post) 
  (advice-add 'add-file-local-variable-prop-line 
              :after #'chunyang-add-file-local-variable-post))

(use-package 
  dired                                 ; Directory Editor
  :defer t 
  :preface (defun chunyang-dired-view-file-other-window () 
             (interactive) 
             (cl-letf (((symbol-function 'view-file) #'view-file-other-window)) 
               (dired-view-file))) 
  :config (bind-key "V" #'chunyang-dired-view-file-other-window dired-mode-map)
...
(use-package 
  ...
  (when *is-mac* 
    (setq dired-guess-shell-alist-user (list (list (rx "." (or
                                                            ;; Videos
                                                            "mp4" 
                                                            "avi"
                                                            "mkv"
...
(use-package 
  direx                                 ; Alternative to Dired
  :ensure t 
  :defer t
  ;;  :bind ("C-x d" . direx:jump-to-directory)
  )
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :init (setq visual-fill-column-width fill-column)
  ;; (setq visual-fill-column-fringes-outside-margins nil)
  (add-hook 'visual-line-mode-hook 
            (defun visual-fill-column-toggle () 
              (visual-fill-column-mode (or visual-line-mode 
                                           -1)))))
...
(use-package 
  ...
  :bind (("M-g c" . avy-goto-char) 
         ("M-g l" . avy-goto-line)) 
  :config (with-eval-after-load "isearch" (bind-key "C-'" #'avy-isearch isearch-mode-map)))
...
(use-package 
  zop-to-char                           ; alternative to `zap-to-char'
  :ensure t
...
(use-package 
  mark-align                            ; Align visualized (via Marking)
...
(use-package 
  whitespace                            ; Highlight bad whitespace (tab)
  :diminish " Whitespace"
...
(put 'view-hello-file 'disabled "I mistype C-h h a lot and it is too slow to block Emacs")
...
;; and smooth
(setq mouse-wheel-progressive-speed nil mouse-wheel-scroll-amount '(1))
...
(use-package 
  ...
  :defer t 
  :preface (defun chunyang-add-hooks (hooks funcs &optional append local) 
             (dolist (hook hooks) 
               (dolist (func funcs) 
  ...
  :init (chunyang-add-hooks '(prog-mode-hook compilation-mode-hook outline-mode-hook help-mode-hook) 
...
(use-package 
  ...
  :no-require t 
  :defer t 
  :preface (setq isearch-allow-scroll t) 
...
Intended to be added to `isearch-mode-hook'."
  ...
  :config (when (version>= emacs-version "27") 
...
(use-package 
  paren                                 ; Highlight paired delimiters
...
(use-package 
  ...
  :config (add-to-list 'hl-todo-keyword-faces '("IDEA" . "#d0bf8f")))
...
(use-package 
  abbrev                                ; For fixing typo only for now
  :disabled t 
  :defer t 
  :init (setq only-global-abbrevs t)
...
(use-package 
  ...
  (setq company-tooltip-align-annotations t company-minimum-prefix-length 2))
...
(use-package 
  langtool                            ; LanguageTool <https://languagetool.org/>
  :homepage https://github.com/mhayashi1120/Emacs-langtool 
  :ensure t 
  :defer t 
  :config (setq langtool-language-tool-jar
                ;; Find newest version
                (car (sort (file-expand-wildcards
                            "~/src/LanguageTool-*/languagetool-commandline.jar") #'string>))))
...
(use-package 
  flycheck-package                      ; Extra Emacs Lisp checker
  :ensure t 
  :after flycheck 
...
(use-package 
  flycheck-elsa-without-cask 
  :after flycheck 
  :no-require t 
  :config (flycheck-define-checker emacs-lisp-elsa-without-cask "Run Elsa without Cask." 
                                   :command ("elsa" (eval (flycheck-prepend-with-option "-L"
                                                                                        load-path))
                                             source) 
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun chunyang-markdown-link (url) 
             ...
             (let (title markdown) 
               ...
               (kill-new markdown) 
               (message "Copied: %s" markdown) markdown)) 
...
unlike `markdown-preview'." 
    ...
    (let ((output (markdown-export))) 
      (if (string-match-p (regexp-quote output) 
                          (chunyang-chrome-url)) 
  ...
  (define-minor-mode chunyang-markdown-preview-in-chrome-mode 
    ...
    (unless (eq major-mode 'markdown-mode) 
      (user-error 
       "Error: %s is not Markdown Mode"
       major-mode)) 
    (if chunyang-markdown-preview-in-chrome-mode (add-hook 'after-save-hook
                                                           #'chunyang-markdown-preview-in-chrome
                                                           :append 
                                                           :local) 
      (remove-hook 'after-save-hook #'chunyang-markdown-preview-in-chrome 
                   :local))))
...
(use-package 
  ...
  (defun chunyang-eldoc-header-line-message (format-string &rest args) 
    (setq header-line-format (apply #'format format-string args)) 
...
(use-package 
  ...
  (setq compilation-always-kill t compilation-scroll-output 'first-error))
...
(use-package 
  chunyang-comment 
  :commands (chunyang-comment-section chunyang-insert-comment-section))
...
(use-package 
  ...
  :config (unbind-key "M-r" paredit-mode-map) 
  (bind-key "M-R" #'paredit-raise-sexp  paredit-mode-map) 
  (unbind-key "M-s" paredit-mode-map) 
  (bind-key "M-S" #'paredit-splice-sexp paredit-mode-map) 
...
(use-package 
  elisp-mode 
  :defer t 
  :preface (defun chunyang-elisp-function-or-variable-quickhelp (symbol) 
             "Display a short documentation of function or variable using `popup'.

See also `describe-function-or-variable'." 
             (interactive (let* ((v-or-f (variable-at-point)) 
               ...
               (let* ((fdoc (when (fboundp symbol) 
                 ...
                 (and 
                  ...
                  (popup-tip (or (and fdoc-short 
                                 ...
                                 vdoc-short) 
                             :margin t))))) 
  :config (bind-key "C-h C-." #'chunyang-elisp-function-or-variable-quickhelp) 
  (bind-key "M-:"     #'pp-eval-expression) 
  (bind-key "C-c t d" #'toggle-debug-on-error) 
  (let ((sym-regexp (or (bound-and-true-p lisp-mode-symbol-regexp) 
                        "\\(?:\\sw\\|\\s_\\|\\\\.\\)+"))) 
    (add-to-list 'lisp-imenu-generic-expression (list "Packages" (concat "^\\s-*("
                                                                         (eval-when-compile
                                                                           (regexp-opt '("use-package"
                                                                                         "require")
                                                                                       t))
                                                                         "\\s-+\\(" sym-regexp
                                                                         "\\)") 2)) 
    (add-to-list 'lisp-imenu-generic-expression (list "Hydras" (concat "^\\s-*(" (eval-when-compile
                                                                                   (regexp-opt '("defhydra")
                                                                                               t))
                                                                       "\\s-+\\(" sym-regexp "\\)")
                                                      2))) 
  (define-advice 
      ...
      (:around (orig-fun sym &rest r) 
               docstring)
    "If SYM is a function, append its docstring."
    (concat (apply orig-fun sym r) 
            (let* ((doc (and (fboundp sym) 
                             (documentation sym 'raw))) 
                   (oneline (and doc 
  ...
  (define-advice 
    ...
    (condition-case err (funcall old-fun) 
      (scan-error (if (nth 4 (syntax-ppss)) 
                      (let ((work-buffer (current-buffer)) 
                            (temp-buffer (generate-new-buffer " *temp*")) found sexp error) 
                        (with-current-buffer temp-buffer (delay-mode-hooks (emacs-lisp-mode))) 
                        (save-excursion (comment-normalize-vars) 
                                        (while (and (comment-beginning) 
                                          ...
                                          (let ((code 
                                                 (buffer-substring-no-properties 
                                                  (point) 
                                                  (line-end-position)))) 
                                            (with-current-buffer temp-buffer (goto-char (point-min)) 
                                                                 (insert code ?\n) 
                                                                 (goto-char (point-max)) 
                                                                 (condition-case err 
                                          ...
                                          (when (= -1 (forward-line -1)) 
                                            (error 
                                             "elisp--preceding-sexp@multiline-comment error")) 
...
(use-package 
  ...
  :config (push 'chunyang-eval-print-last-sexp aggressive-indent-protected-commands) 
  (push 'chunyang-macroexpand-print-last-sexp aggressive-indent-protected-commands))

(use-package 
  el-search 
  :ensure t 
  :defer t 
  :preface (defun chunyang-el-search-symbol-or-sexp-at-point ()
             ;; Adopted from `el-search-this-sexp'
             (when (derived-mode-p 'emacs-lisp-mode) 
               (let ((symbol-at-point-text (thing-at-point 'symbol)) symbol-at-point) 
                 (if (and symbol-at-point-text
                          ;; That should ideally be always true but isn't
                          (condition-case nil (symbolp 
                                               (setq symbol-at-point (read symbol-at-point-text))) 
                            (invalid-read-syntax nil))) `',symbol-at-point (when (thing-at-point 'sexp) `',(sexp-at-point)))))) 
  (defun chunyang-el-search-git-repo (directory pattern) 
    "El-search all elisp files in current Git repository." 
    (interactive (progn 
                   (require 'el-search) 
                   (require 'magit) 
                   (let* ((directory (or (magit-toplevel)
                                         ;; Limited to `magit-repository-directories'
                                         (magit-read-repository))) 
                          (default (format "%s" (chunyang-el-search-symbol-or-sexp-at-point))) 
                          (input (el-search--read-pattern (format "El-search %s for pattern: "
                                                                  (abbreviate-file-name directory))
                                                          default)) 
                          (pattern (read input))) 
                     (list directory pattern)))) 
    (el-search-setup-search pattern 
                            (lambda () 
                              (stream (seq-filter 
                                       (lambda (s) 
                            ...
                            (lambda (search) 
                              (setf (alist-get 'description (el-search-object-properties search)) 
                                    (concat "Search the git repo in " (abbreviate-file-name
                                                                       directory)))))) 
...
PACKAGE should not be a built-in package." 
    (interactive (list (el-search-read-pattern-for-interactive "Search package for pattern: ") 
                       (intern (completing-read "el-search package: " (mapcar 'car package-alist)
                                                nil t)))) 
    (let ((pkgdir (package-desc-dir (cadr (assq package package-alist))))) 
      (el-search-directory pattern pkgdir t))) 
  (defun chunyang-el-search-elpa (pattern) 
    (interactive (let* ((default (format "%s" (chunyang-el-search-symbol-or-sexp-at-point))) 
                        (input (el-search--read-pattern (format "El-search %s for pattern: "
                                                                (abbreviate-file-name
                                                                 package-user-dir)) default)) 
                        (pattern (read input))) 
                   (list pattern))) 
  ...
  ;; even in Emacs -Q.
  (with-eval-after-load 'elisp-mode (el-search-install-shift-bindings)) 
  :config (defun el-search-helm () 
            (interactive) 
            ...
            (let* ((search el-search--current-search) 
                   ...
                   (candidates (mapcar (pcase-lambda ((and 
                                                       match
                                                       `(,buffer ,match-beg ,file))) 
                                         (with-current-buffer buffer (cons 
                                                                      (buffer-substring-no-properties 
                                                                       match-beg
              ...
              (require 'helm) 
              (helm :sources (helm-build-sync-source (el-search--get-search-description-string
  ...
  (defun chunyang-el-search-change-occur-buffer-name () 
    (when el-search-occur-search-object (setf (buffer-name) 
                                              (generate-new-buffer-name (format "*El Occur: %s*"
                                                                                (cl-substitute ?\s
                                                                                               ?\n
                                                                                               (prin1-to-string
      ...
      (mapatoms 
       (lambda (sym) 
         (and (fboundp sym) 
              ...
              (push sym symbols)))) symbols) 
    "A list of symbols which support doc-string.") 
  (defun el-search--doc-string-p () 
    "Return t if point is at docstring."
    (pcase (save-excursion (backward-up-list) 
                           (read (current-buffer))) 
      (`(,(and symbol 
               (guard (memq symbol el-search--symbols-with-doc-string))) ,_ . ,_) 
       (let ((op (point))) 
         (save-excursion (backward-up-list) 
                         (forward-char) 
                         (ignore-errors (forward-sexp (1+ (get symbol 'doc-string-elt))) 
                                        (backward-sexp) 
                                        (= op (point))))))))

  ;; Type C-h f `el-search-defined-patterns' to learn defined patterns
  (el-search-defpattern doc-string 
      (&rest 
       regexps)
    "Match any documentation string that is matched by all REGEXPS." `(and (string ,@regexps) 
                                                                           (guard
                                                                            (el-search--doc-string-p)))) 
  (el-search-defpattern s 
      (&rest 
       regexps)
    "Match any string (excluding doc string) that is matched by all REGEXPS" `(and (string
                                                                                    ,@regexps) 
                                                                                   (guard (not
                                                                                           (el-search--doc-string-p))))))
...
(use-package 
  chunyang-elisp 
  :commands (chunyang-format-command-on-key chunyang-format-help-on-key
                                            ...
                                            chunyang-display-window-mode chunyang-toggle-setq-form
                                            chunyang-insert-command chunyang-insert-key) 
  :preface (defun threadify (exp) 
...
(use-package 
  ...
  :init (when (>= emacs-major-version 25) 
          (advice-add 'describe-package-1 
                      :after #'describe-package--add-melpa-link)))

(use-package 
  ielm 
  :defer t 
  :config (add-hook 'ielm-mode-hook #'enable-paredit-mode))
...
(use-package 
  bug-hunter                            ; This is good
  :disabled t 
  :ensure t 
  :defer t)

(use-package 
  debbugs                               ; Interface to GNU Bugs
  :ensure t 
  :defer t 
  :preface
  ...
  (defun chunyang-open-emacs-bug (id) 
    ...
    (interactive (list (or (let ((case-fold-search t)) 
                             (when (thing-at-point-looking-at (rx (opt "Bug#") 
                                                                  (group (1+ num))) 10) 
                               (string-to-number (match-string 1)))) 
                           (read-number "Bug Id: ")))) 
    (let ((url (format "https://debbugs.gnu.org/cgi/bugreport.cgi?bug=%s" id))) 
...
(use-package 
  ...
  (defmacro DEBUG 
      (&rest 
       exprs) 
    `(message ,(concat "[DEBUG] " (mapconcat 
                                   (lambda (e) 
                                     (concat (replace-regexp-in-string "%" "%%" (prin1-to-string e))
                                             " = %S"))
                                   exprs ", ")) ,@exprs)) 
  :config (defhydra hydra-debugger-menu () "Debug" ("c" debugger-continue "Continue") 
...
(use-package 
  package-lint                          ; Check Compatibility
  :ensure t 
  :defer t)
...
(use-package 
  help 
  :preface (defun view-help-buffer () 
  ...
  (defun chunyang-describe-symbol-at-point () 
    ...
    (interactive) 
    (require 'help-mode) 
    (describe-symbol (or (pcase (variable-at-point) 
                           (0 nil) 
                           (v v)) 
                         ...
                         (let* ((is-symbol-p 
                                ...
                                (sym (or (let 
                                             ((it 
                                               (intern (current-word)))) 
                                           (when (funcall is-symbol-p it) it)) 
                                         (completing-read "Describe symbol: " obarray is-symbol-p t)))) sym)))) 
  (defun chunyang-advice-remove-button (function) 
    "Add a button to remove advice."
    (when (get-buffer "*Help*") 
      (with-current-buffer "*Help*" (save-excursion (goto-char (point-min))
  ...
  :bind (("C-h ." . chunyang-describe-symbol-at-point) 
         ...
         :map help-mode-map ("b" . help-go-back) 
         ("f" . help-go-forward) 
         ("i" . help-info-lookup-symbol)) 
  :config (temp-buffer-resize-mode) 
  (advice-add 'describe-function-1 
              :after #'chunyang-advice-remove-button))

(use-package 
  info-look 
  :defer t 
  :config (info-lookup-add-help :mode 'emacs-lisp-mode 
                                :regexp "[^][()`'‘’,\" \t\n]+" 
                                :doc-spec '(("(emacs)Command Index"             nil
                                             "['`‘]\\(M-x[ \t\n]+\\)?" "['’]") ;
...
(use-package 
    ...
    '(
      ...
      ("geiser" . 
       (lambda (_)
         ...
         (if (string=  Info-current-node "Top") "http://www.nongnu.org/geiser/" (let ((node
                                                                                      ...
                                                                                      (number
                                                                                       (save-excursion
                                                                                         ...
                                                                                         (match-string
                                                                                          0)))) 
                                                                                  (format
                                                                                   "http://www.nongnu.org/geiser/geiser_%s.html#%s"
                                                                                   number node))))) 
      ...
      (("awk" "sed" "tar" "make" "m4" "grep" "coreutils" "guile" "screen" "libc" "make" "gzip"
        "diffutils" "wget" "grub") . 
        (lambda (software) 
      ...
      (("ada-mode" "auth" "autotype" "bovine" "calc" "ccmode" "cl-lib" "dbus" "dired-x" "ebrowse"
        "ede" "ediff" "edt" "efaq-w32" "efaq" "eieio" "eintr" "elisp" "emacs-gnutls" "emacs-mime"
        "emacs" "epa" "erc" "ert" "eshell" "eudc" "eww" "flymake" "forms" "gnus" "htmlfontify"
        "idlwave" "ido" "info" "mairix-el" "message" "mh-e" "newsticker" "nxml-mode" "octave-mode"
        "org" "pcl-cvs" "pgg" "rcirc" "reftex" "remember" "sasl" "sc" "semantic" "ses" "sieve"
        "smtpmail" "speedbar" "srecode" "todo-mode" "tramp" "url" "vip" "viper" "widget" "wisent"
        "woman") . 
        (lambda (package) 
  ...
  (defun chunyang-org-info-map-anchor-url (node) 
    ...
    (let* ((node (replace-regexp-in-string "[ \t\n\r]+" " " (org-trim node))) 
           ...
           (url (if (string= node "") "" (if (string-match "[0-9]" (substring node 0 1)) 
                                             (concat "g_t" node) node)))) url)) 
  (defun chunyang-Info-get-current-node-html () 
    (cl-assert 
     (eq major-mode 'Info-mode)) 
    (let* ((file (file-name-nondirectory Info-current-file)) 
           (node Info-current-node) 
           (html (if (string= node "Top") "" (concat (chunyang-org-info-map-anchor-url node)
                                                     ".html"))) 
           (baseurl (cl-loop for (k . v) in chunyang-Info-html-alist when (cond ((stringp k) 
                                                                                 (equal file k)) 
                                                                                ((listp k) 
                                                                                 (member file k)))
...
(use-package 
  cus-edit 
  :preface (defun chunyang/custom-mode-describe-symbol-at-point () 
             (interactive) 
             (require 'info-look) 
...
(use-package 
  command-log-mode                     ; BUG: Create a new empty buffer and
...
(use-package 
  ...
  :config (setq git-gutter:handled-backends '(git svn)) 
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun helm-gitignore-template () 
             (interactive) 
             (require 'helm) 
             (helm :sources (helm-build-in-buffer-source "Gitignore Templates" 
                              :data (split-string (shell-command-to-string
                                                   "curl -s https://api.github.com/gitignore/templates | jq -r '.[]'") "\n") 
                              :action (let ((new-action 
                                             (lambda (fun) 
                                               (lambda (lang) 
                                                 (funcall fun (shell-command-to-string (format
                                                                                        "curl -s https://api.github.com/gitignore/templates/%s | jq -r '.source'"
                                                                                        lang))))))) 
                                        (helm-make-actions "Insert" (funcall new-action #'insert)
...
(use-package 
  ediff 
  :defer t 
  :init (defun chunyang-dired-ediff (file-a file-b) 
...
(use-package 
  ...
  :ensure nil 
  :defer 3 
  :config (autoload 'server-running-p "server") 
  ...
  (when *is-mac*
    ...
    (setq server-temp-file-regexp (rx string-start (eval temporary-file-directory)))) 
...
(use-package 
  ...
  :commands (helm-chunyang-github-stars helm-chunyang-github-repos) 
  :preface (defun chunyang-github-create () 
             ...
             (let* ((git-root (or (locate-dominating-file default-directory ".git") 
               ...
               (call-process-shell-command (concat "git remote add origin " (alist-get 'clone_url
                                                                                       response))) 
  ...
  (defun chunyang-edit-file-as-root () 
    ...
    (let ((file buffer-file-name)) 
      (and file 
           ...
           (find-alternate-file (tramp-make-tramp-file-name (file-remote-p file 'method) "root"
...
This should be add to `find-file-hook'."
    (let ((old-msg (current-message))) 
      (when (and old-msg 
        ...
        (message "%s, %s" old-msg "use M-x sudo-edit RET to edit via sudo")))) 
...
(use-package 
  ...
  :commands dashdoc 
  :config (ivy-set-actions 'dashdoc-ivy '(("b" 
                                           (lambda (x) 
...
(use-package 
  rcirc 
  :commands rcirc 
  :config (setq rcirc-default-nick "chunyang") 
  (setq rcirc-server-alist '(("irc.freenode.net" :channels ("#emacs")) 
  ...
  (defun wh/log-filename-with-date (process target) 
    (format "%s_%s.log" (if target (rcirc-generate-new-buffer-name process target) 
...
(use-package 
  erc 
  :defer t 
  :config (setq erc-track-exclude-types '("JOIN" "NICK" "PART" "QUIT")))
...
(setq user-full-name       "fengshuhao" user-mail-address    "274757565@qq.com"
      ;; This is required for ~/.authinfo.gpg but not ~/.authinfo
      smtpmail-smtp-user   user-mail-address smtpmail-smtp-server "smtp.fastmail.com"
      smtpmail-smtp-service 465 smtpmail-stream-type 'ssl send-mail-function   'smtpmail-send-it)

(use-package 
  message 
  :defer t 
  :functions epa-mail-default-recipients 
  :notes ((message-elide-region         . "C-c C-e 省略一段冗长的引用") 
          (message-mark-inserted-region . "C-c M-m 给一段文字（常常是代码）加上框")) 
  :preface (defun chunyang-message-signature () 
             ...
             (let ((real-recipients (epa-mail-default-recipients))) 
               (cond ((member "emacs-orgmode@gnu.org" real-recipients) 
                     ...
                     ((member "emacs-devel@gnu.org" real-recipients) nil) 
                     (t nil)))) 
  :config
...
(use-package 
  ...
  :commands notmuch 
  :preface (defun chunyang-notmuch-update () 
             (interactive) 
             (shell-command "notmuch tag -unread -inbox -- tag:unread \
...
proxychains4 mbsync --verbose --all && notmuch new&") 
             (when-let ((buffer (get-buffer "*notmuch-hello*"))) 
               (with-current-buffer buffer (notmuch-refresh-this-buffer)))) 
...
(use-package 
  mu4e 
  :commands mu4e 
  :config (add-to-list 'mu4e-view-actions '("open in web browser" .
                                            chunyang-mu4e-action-open-html-in-browser)) 
  (defun chunyang-mu4e-action-open-html-in-browser (msg) 
    (when-let* ((html (mu4e-message-field msg 
                                          :body-html)) 
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :config (setq twittering-proxy-use t twittering-proxy-server "127.0.0.1" twittering-proxy-port
                1087))
...
(use-package 
  ...
  :ensure t 
  :commands elfeed 
  :config (setq elfeed-curl-extra-arguments '("--proxy" "socks5://127.0.0.1:1080" "--retry" "3")) 
  (setq elfeed-curl-timeout 60)
  ;; TODO 用 Org Mode 管理 Feeds
  ;; - Elfeed
  ;; - OPML
  ;; - JSON
  (setq elfeed-feeds '("http://nullprogram.com/feed/" ("http://emacsninja.com/emacs.atom" emacs) 
                       ("http://emacshorrors.com/feed.atom" emacs) 
                       ("http://planet.emacsen.org/atom.xml" emacs) 
                       ("https://emacs-china.org/latest.rss" emacs) 
                       ("http://ubuntupodcast.org/feed/" ubuntu) "https://sspai.com/feed"
                       "https://sirsharpest.github.io/rss.xml"
                       "http://feeds2.feedburner.com/stevelosh" "http://feed.williamlong.info/"
                       "http://matt.might.net/articles/feed.rss"
                       "https://olivierpieters.be/feed.xml" "https://two-wrongs.com/feed.xml"
                       "http://alex-charlton.com/rss.xml" "https://jameshfisher.com/feed.xml"
                       "https://open.nytimes.com/feed" "http://bluxte.net/rss.xml"
...
(use-package 
  ...
  (advice-add 'url-cookie-setup-save-timer 
              :override #'ignore))
...
(use-package 
  ...
  (when (version< emacs-version "26") 
    (with-eval-after-load 'eww (advice-add 'eww-display-html 
                                           :around
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :config (add-to-list 'Info-directory-list "~/src/emms/doc") 
  (setq emms-mode-line-icon-color "purple") 
  (require 'emms-setup) 
  (emms-all) 
  (emms-playing-time -1) 
  (setq emms-player-list '(emms-player-mplayer) emms-source-file-default-directory
        "~/Music/网易云音乐" emms-source-file-gnu-find "gfind") 
  (require 'emms-info-libtag) 
  (setq emms-info-functions '(emms-info-libtag)) 
  (defun chunyang-emms-indicate-seek (_sec) 
    (let* ((total-playing-time (emms-track-get (emms-playlist-current-selected-track)
                                               'info-playing-time)) 
           (elapsed/total (/ (* 100 emms-playing-time) total-playing-time))) 
      (with-temp-message (format "[%-100s] %2d%%" (make-string elapsed/total ?=) elapsed/total) 
        (sit-for 2)))) 
...
(use-package 
  ...
  (setq bongo-default-library-buffer-name " *Bongo Library*" bongo-default-playlist-buffer-name
        " *Bongo Playlist*") 
...
(defun helm-music-player () 
  ...
  (interactive) 
  (require 'helm) 
  (helm :sources (helm-build-in-buffer-source "Music" 
                   :data (mapcar #'abbreviate-file-name (split-string (shell-command-to-string
                                                                       "find ~/Music -name '*.mp3'")
                                                                      "\n")) 
                   :action (lambda (mp3) 
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun bing-dict-eldoc-documentation-function () 
             (let ((word (word-at-point)))
               ;; 太短的单词不查
               (when (and word 
                          (> (length word) 4)) 
                 (bing-dict-brief word)) nil))

  ;; 注意一次只有一个 eldoc mode backend 生效
  (define-minor-mode bing-dict-eldoc-mode "Use bing-dict as backend of eldoc." 
    :lighter " Bing Dict"
    (if bing-dict-eldoc-mode (progn (setq-local eldoc-documentation-function
...
(use-package 
  ...
  :preface (defun chunyang-google-translate-web (query) 
             "Launch Google Translate with Web Browser." 
             (interactive (let* ((default (or (and (use-region-p) 
                                                   (buffer-substring 
                                                    (region-beginning) 
                                                    (region-end))) 
                                              (current-word))) 
                                 (prompt (if default (format "Google Translate (default %s): "
                                                             default) "Google Translate: "))) 
                            (list (read-string prompt nil nil default)))) 
             (browse-url (format "https://www.google.com/translate_t?text=%s" (url-hexify-string
                                                                               query)))) 
  :init (eval-after-load 'google-translate-core 
...
(defun chunyang-moedict (query) 
  (interactive (let ((char (char-after))) 
                 (list (read-string "萌典: " (and char 
...
(use-package 
  flycheck-checkbashisms               ; Don't use Bash-only features in /bin/sh
  :disabled Use 
  ShellCheck
  instead 
...
(use-package 
  ...
  (defun chunyang-eshell-insert-last-arg () 
    ...
    (interactive) 
    (with-current-buffer eshell-buffer-name (let ((last-arg (car (last (split-string
  ...
  :init (setq eshell-banner-message '(concat (mapconcat #'identity (mingju) " -- ") "\n\n")) 
  :config (setq eshell-history-size 5000) ; Same as $HISTSIZE
  ...
  (add-hook 'eshell-mode-hook 
            (defun chunyang-eshell-mode-setup ()
              ...
              (bind-keys :map eshell-mode-map ("C-c C-q" . eshell-kill-process) 
  ...
  (use-package 
    ...
    :disabled t 
    :ensure t 
    :config (setq eshell-highlight-prompt nil eshell-prompt-function 'epe-theme-lambda)) 
  (use-package 
    chunyang-eshell-ext))
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :init (autoload 'eshell-did-you-mean-setup "eshell-did-you-mean") 
  (with-eval-after-load 'eshell (eshell-did-you-mean-setup)))
...
(use-package 
  ...
  :commands (shell/info                 ; For shell in Terminal (outside Emacs)
             helm-bash-history chunyang-concat-shell-command))
...
(defcustom server-eval-and-how-to-print nil 
  ...
  :type '(choice (const :tag "Default (that is, print result of evaluation)" nil) 
...
(define-advice 
  ...
  (let ((server-eval-and-how-to-print server-eval-and-how-to-print)) 
    (seq-let (_orig-fun expr proc) r (let ((v (with-local-quit (eval (car (read-from-string
                                                                           expr)))))) 
                                       (pcase server-eval-and-how-to-print ('supress nil) 
                                              ('buffer (when proc 
                                                         (require 'e2ansi) 
                                                         (server-reply-print (server-quote-arg
                                                                              (e2ansi-string-to-ansi
                                                                               (buffer-string)))
                                                                             proc))) 
                                              (_ (when proc (with-temp-buffer (let ((standard-output
                                                                                     (current-buffer))) 
                                                                                (pp v) 
                                                                                (let ((text 
                                                                                       (buffer-substring-no-properties 
...
(use-package 
  org 
  :preface (defun chunyang-org-eval-defun () 
  ...
  (defun chunyang-org-info-lookup-symbol () 
    ...
    (if (not (org-in-src-block-p)) 
      ...
      (org-babel-do-in-edit-buffer (save-excursion (call-interactively 'info-lookup-symbol))) 
      (switch-to-buffer-other-window "*info*"))) 
  (defun chunyang-org-babel-open-tangle-file () 
    (interactive) 
    (unless (org-in-src-block-p) 
      (user-error 
       "Point is not in a source code block")) 
    (let* ((spec (car (cdr (car (org-babel-tangle-single-block 1 t))))) 
           (get-spec 
            (lambda (name) 
              (cdr (assoc name (nth 4 spec))))) 
           (tangle (funcall get-spec 
                            :tangle))) 
      (when tangle (find-file tangle)))) 
...
Adapt from `org-babel-remove-result'." 
    ...
    (let ((location (org-babel-where-is-src-block-result nil nil))) 
      (when location (save-excursion (goto-char location) 
                                     (when (looking-at (concat org-babel-result-regexp ".*$")) 
                                       (pulse-momentary-highlight-region (1+ (match-end 0)) 
  ...
  (defun chunyang-org-babel-copy-previous-src-block ()
    ...
    (interactive) 
    (let (result) 
      (save-excursion (org-babel-previous-src-block) 
                      (let ((element (org-element-at-point))) 
                        (when (eq (car element) 'src-block) 
                          (let* ((pl (cadr element)) 
                                 (lang (plist-get pl 
                                                  :language)) 
                                 (switches (plist-get pl 
                                                      :switches)) 
                                 (parms (plist-get pl 
                                                   :parameters))) 
                            (setq result (format (concat "#+begin_src %s\n" "\n" "#+end_src\n") 
                                                 (mapconcat #'identity (delq nil (list lang switches
                                                                                       parms)) "
"))))))) 
      (and result 
           (insert result)) 
  ...
  (defun chunyang-org-babel-execute-python-in-iTerm () 
    ...
    (seq-let (type plist) 
      ...
      (cond ((not (eq 'src-block type)) 
             (user-error 
              "Not a src block")) 
            ((not (string= "python" (plist-get plist 
                                               :language))) 
             (user-error 
              "Not a Python src block")) 
            (t (let ((filename (make-temp-file "" nil ".py"))) 
  ...
  (defun chunyang-org-babel-execute-js-in-iTerm () 
    ...
    (seq-let (type plist) 
      ...
      (cond ((not (eq 'src-block type)) 
             (user-error 
              "Not a src block")) 
            ((not (string= "js" (plist-get plist 
                                           :language))) 
             (user-error 
              "Not a JS src block")) 
            (t (let ((filename (make-temp-file "" nil ".js"))) 
                 (org-babel-tangle '(4) filename) 
  ...
  :config (bind-key "C-M-x" #'chunyang-org-eval-defun org-mode-map)
  ...
  ;; 设置org文件位置
  (setq org-agenda-files '("~/Dropbox/feng/200_hufflepuff/210_plan/210.03_gtd"))
  ;; 开启时间网格
  ...
  (setq org-default-notes-file (concat org-directory "/210.02_today.org"))
  ...
  (setq org-refile-targets (quote (("task.org" :maxlevel . 1) 
                                   ("someday.org" :level . 2)))) 
  (setq org-agenda-custom-commands '(("h" "Office and Home Lists" ((agenda) 
                                                                   (tags-todo "OFFICE") 
                                                                   (tags-todo "HOME"))) 
                                     ("d" "Daily Action List" ((agenda "" ((org-agenda-ndays 1) 
  ...
  (setq org-capture-templates `(("t" "Todo" entry (file
                                                   "~/Dropbox/feng/200_hufflepuff/210_plan/task.org")
                                 "* TODO %^{Brief Description} %^g\n%?Added: %U" 
                                 :empty-lines 1) 
                                ("j" "Journal" entry (file+headline
                                                      "~/Dropbox/feng/200_hufflepuff/210_plan/journal.org"
                                                      ,(format-time-string "%b %Y" (current-time)))
                                 "* %^{Brief Description} %^g\n%?\nAdded: %U" 
                                 :empty-lines 1) 
                                ("s" "someday" entry (file
                                                      "~/Dropbox/feng/200_hufflepuff/210_plan/someday.org") "* %?\nAdded: %U" 
                                                        :empty-lines 1)
  ...
  (defun chunyang-org-capture () 
    (interactive) 
    (let* ((input (completing-read "org capture template:" (mapcar 
                                                            (lambda (template) 
                                                              (format "%s %s" (car template) 
                                                                      (cadr template)))
  ...
  (setq org-protocol-protocol-alist '(("bookmark" :protocol "bookmark" 
  ...
  (defun chunyang-org-protocol-capture-bookmark (_info) 
    ...
    (when *is-mac-port* (run-at-time 0 nil #'chunyang-mac-switch-back-to-previous-application))
    nil) 
  (defun chunyang-mac-switch-back-to-previous-application () 
    ...
    (do-applescript (mapconcat #'identity '("tell application \"System Events\""
                                            "  tell process \"Finder\"" "    activate"
...
(use-package 
  chunyang-org 
  :commands (chunyang-org-agenda-csv helm-org-easy-templates
                                     chunyang-org-format-region-as-code-block
                                     chunyang-org-preview-via-pandoc chunyang-org-babel-tangle))
(use-package 
  toc-org 
...
(use-package 
  htmlize                               ; Enable src block syntax
...
(use-package 
  ...
  ;; /*** includes ***/
  (add-to-list 'cc-imenu-c-generic-expression (list "Section" (rx line-start "/***" (group (1+
                                                                                            not-newline))
                                                                  "***/" line-end) 1)))
(use-package 
  irony 
  ...
  :config (use-package 
            ...
            ;; Prefer "fun ()" over "fun()"
            (define-advice 
                company-irony--post-completion 
                (:override (candidate) 
                           put-space-open-parentheses-maybe)
  ...
  (use-package 
    irony-eldoc                         ; Note: this does not work very well
    :ensure t 
    :defer t 
...
(use-package 
  flycheck-rust                ; The built-in check of Flycheck not working well
  :ensure t 
  :defer t 
  :after flycheck 
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun chunyang-sly-eval-print-last-sexp () 
             ...
             (let ((string (sly-last-expression))) 
               (sly-eval-async `(slynk:eval-and-grab-output ,string) 
                               (lambda (result) 
                                 (cl-destructuring-bind (_output value) result (unless
                                                                                   (chunyang-current-line-empty-p) 
                                                                                 (insert ?\n))
                                                        ;; (insert "     ⇒ " value)
                                                        (insert "     => " value) 
                                                        (unless (chunyang-current-line-empty-p) 
                                                          (insert ?\n))))))) 
  (defun chunyang-sly-eval-print-last-sexp-in-comment () 
    ...
    (let ((string (sly-last-expression))) 
      (sly-eval-async `(slynk:eval-and-grab-output ,string) 
                      (lambda (result) 
                        (cl-destructuring-bind (_output value) result (comment-dwim nil) 
...
(use-package 
  ruby-mode 
  :defer t 
  :init (add-hook 'ruby-mode-hook #'superword-mode))
(use-package 
  inf-ruby 
  :ensure t
...
(use-package 
  ...
  :preface (defun chunyang-geiser-eval-print-last-sexp () 
             (interactive) 
               ...
               (let ((res (geiser-eval-last-sexp nil))) 
                 (unless (string= "=> " res) 
                   ...
                   (insert ";; "
                           ...
                           (replace-regexp-in-string "\n=>" "\n;; =>" res) "\n") 
                   (message nil))))) 
  (defun chunyang-geiser-expand-sexp-at-point 
      (&optional 
       all) 
    (interactive "P") 
  ...
  (defun chunyang-geiser-setup () 
    (bind-keys :map geiser-mode-map ("C-h ."   . geiser-doc-symbol-at-point) 
  ...
  (and *is-mac* 
       (setq geiser-image-viewer "open")))
(use-package 
  ob-scheme 
  :defer t 
  :config (define-advice 
              org-babel-scheme-get-repl 
              (:around (old-fun &rest args) 
                       dont-switch-buffer)
...
(use-package 
    ...
    (run-at-time 0 nil 
                 (lambda () 
                   ...
                   (when-let ((buffer (get-buffer "*Racket Describe*"))) 
                     (with-current-buffer buffer (save-excursion (goto-char (point-max)) 
                                                                 (goto-char
  ...
  (defun chunyang-racket-eval-print-last-sexp () 
    (interactive) 
    (let* ((end (point)) 
           (beg (save-excursion (backward-sexp) 
  ...
  ;; This is annoying!
  (advice-add 'racket--repl-show-and-move-to-end 
              :override #'ignore))
(use-package 
  scribble-mode 
...
(use-package 
  sgml-mode 
  :preface (defun chunyang-html-mode-setup ()
...
(use-package 
  js 
  :defer t 
  :preface (defun chunyang-nodejs-find-module (module) 
             (interactive (list (read-string "Module: "
                                             ;; Use `syntax-ppss' or text props?
                                             (thing-at-point 'symbol)))) 
             (let (filename) 
               (with-temp-buffer (let ((exit (call-process "node" nil t nil "-pe" (format
                                                                                   "require.resolve('%s')"
                                                                                   module)))) 
                                   (when (eq (char-before) ?\n) 
                                     (delete-char -1)) 
  ...
  (defun chunyang-js-mode-setup () 
    (setq electric-layout-rules (seq-remove 
                                 (lambda (elt) 
                                   (= (car elt) ?\;))
...
(use-package 
  ...
  (defun tern-describe () 
    ...
    (tern-run-query 
     (lambda (data)
       ;; url, doc, type, origin
       (let-alist data (with-current-buffer (get-buffer-create "*Tern Describe*") 
                         (let ((inhibit-read-only t)) 
                           (erase-buffer) 
                           (when .doc (insert .doc) 
...
(use-package 
  ...
  :ensure t 
  :preface (defun simple-http-server (root-dir) 
             "简单的文件 HTTP 服务器，效果类似于 python -m http.server 8888." 
             (interactive (list (expand-file-name default-directory))) 
             (ws-start 
              `(lambda (request) 
                 (with-slots ((proc process) headers) request (let* ((path (alist-get :GET headers)) 
...
(use-package 
  ...
  :defer t 
  :config (require 'comint) 
  (define-derived-mode inferior-elixir-mode comint-mode 
    "Inferior Elixir"
    "Major mode for Elixir inferior process." 
    (setq comint-prompt-regexp (rx bol (or "iex" 
                                           "...") "(" (1+ num) ") "))) 
  (defun run-elixir () 
    (interactive) 
...
(use-package 
  ...
  (define-advice 
      ...
      (:around (old-fun thing pos eldoc-info) 
               docstring)
    "Show docstring for function as well."
    (concat (funcall old-fun thing pos eldoc-info) 
...
(use-package 
  python 
  :defer t 
  :preface (defun chunyang-python-mode-setup () 
             (eldoc-mode -1) 
             (kill-local-variable 'completion-at-point-functions)) 
  (defun chunyang-inferior-python-mode-setup () 
    (setq-local comint-process-echoes t) 
    (kill-local-variable' completion-at-point-functions)) 
  :config (add-hook 'python-mode-hook #'chunyang-python-mode-setup) 
  (add-hook 'inferior-python-mode-hook #'chunyang-inferior-python-mode-setup) 
  (setq python-indent-guess-indent-offset nil) 
  (setq python-shell-interpreter "python3" python-shell-completion-native-enable nil
        python-shell-font-lock-enable nil))
(use-package 
  chunyang-python 
  :commands (chunyang-jedi chunyang-python-comment-box))
(use-package 
  elpy 
  ...
  t 
  :defer t 
  :init (defun chunyang-elpy-enable () 
          (elpy-enable) 
          (elpy-mode) 
          (remove-hook 'python-mode-hook #'chunyang-elpy-enable)) 
  (add-hook 'python-mode-hook #'chunyang-elpy-enable) 
  :config (setq elpy-modules '(elpy-module-sane-defaults elpy-module-company elpy-module-eldoc)) 
...
(use-package 
  ...
  :defer t)
(use-package 
  e2ansi                                ; Provide Syntax Highlight for shell by
...
(use-package 
  ...
  :ensure t 
  :defer t 
  :preface (defun chunyang-language-detection-region (b e) 
             (interactive "r") 
             (message "Language: %s" (language-detection-string 
                                      (buffer-substring 
                                       b
                                       e)))))
...
(use-package 
  chunyang-chinese 
  :commands (chunyang-chinese-insert-mark chinese-punctuation-mode chunyang-pinyin-occur))
...
(use-package 
  ...
  :config (cond (*is-mac*
                 ...
                 (setq fortune-dir  "/usr/local/share/games/fortunes/" fortune-file
                       "/usr/local/share/games/fortunes/fortunes")) 
                (*is-gnu-linux* 
                 (setq fortune-dir  "/usr/share/games/fortunes/" fortune-file
...
(defun chunyang-birthday-p () 
  ...
  (require 'cal-china) 
  (pcase-let ((`(_ _ ,m ,d) 
               (calendar-chinese-from-absolute (calendar-absolute-from-gregorian
                                                (calendar-current-date)))))
    ;; Note: For leap months M is a float.
    (equal (list (floor m) d) 
           '(9 23))))
...
(use-package 
  ...
  very
  impressive 
  :preface (autoload 'epkg-read-package "epkg") 
  (defun chunyang-use-package (package) 
    (interactive (list (epkg-read-package "Insert use-pacakge form for package: "))) 
    (pp 
     `(use-package 
        ,(intern package) 
...
(use-package 
  ...
  (defun chunyang-elixir-current-project (dir) 
    ...
    (let ((project-dir (locate-dominating-file dir "mix.exs"))) 
      (if project-dir (cons 'elixir project-dir) nil))) 
...
(use-package 
  ...
  :init (add-hook 'c-mode-hook #'lsp-cquery-enable) 
...
(use-package 
  ...
  :about "XML writing" 
  :notes
  "Unlike esxml, xmlgen uses its own sexp syntax and don't with work libxml-parse-html-region" 
  :ensure t 
  :defer t)
(use-package 
  chart                                 ; Built-in
...
(defun s-font () 
  (set-face-attribute 'default nil 
                      :font (font-spec :name
                                       "-*-Monaco-normal-normal-normal-*-*-*-*-*-m-0-iso10646-1" 
                                       :weight 'normal 
                                       :slant 'normal 
                                       :size 12.5)) 
  (dolist (charset '(kana han symbol cjk-misc bopomofo)) 
    (set-fontset-font (frame-parameter nil 'font) charset (font-spec :name
                                                                     "-*-Source Han Serif-normal-normal-normal-*-*-*-*-*-p-0-iso10646-1" 
                                                                     :weight 'normal 
                                                                     :slant 'normal 
                                                                     :size 14.5))))
...
(defun sql-beautify-region (beg end) 
  ...
  (interactive "r") 
  (save-excursion (shell-command-on-region beg end "beautify-sql" nil t)))
...
(defun sql-beautify-region-or-buffer () 
  ...
  (interactive) 
  (if (use-region-p) 
      (sql-beautify-region (region-beginning) 
                           (region-end)) 
    (sql-beautify-buffer)))
...
(defun yp-mark-line 
  ...
  (interactive "P") 
  (if (region-active-p) 
      (progn (goto-char (line-end-position 2))) 
  ...
  (if (and arg 
           (> arg 1)) 
      (progn (goto-char (line-end-position arg)))))
...
With argument ARG, do this that many times." 
  (interactive "p") 
  (kill-region (progn (backward-word) 
                      (point)) 
                          ...
                          (mapc 
                           (lambda ($fpath) 
                             (when (buffer-modified-p ) 
                               (save-buffer)) 
                             (shell-command (format "open -a Safari.app \"%s\"" $fpath)))
                           $file-list))))))
...
(defun indent-region-or-buffer() 
  ...
  (save-excursion (if(region-active-p) 
                      (progn (indent-region(region-beginning) 
                                           (region-end)) 
...
(use-package 
  ...
  (use-package 
    ispell 
    :defer t 
    :init (setq ispell-program-name "aspell" ispell-extra-args '("--sug-mode=fast"
...
(use-package 
  ...
  (setq calendar-holidays (append cal-china-x-important-holidays cal-china-x-general-holidays)))
...
;; fk 不能同时移动多个??
(defun movefile (file) 
  (make-directory (file-name-directory (replace-regexp-in-string "/Dropbox/feng/"
                                                                 "/Dropbox/hogwarts/" file)) t) 
  (rename-file file (concatenate 'string (file-name-sans-extension (replace-regexp-in-string
                                                                    "/Dropbox/feng/"
                                                                    "/Dropbox/hogwarts/" file)) "_"
                                                                    (format-time-string "%Y%m%d")
...
(defun archive-notes 
    (&optional 
     arg)
  "move flie to archive file" 
  (interactive) 
  (if (string-equal major-mode "dired-mode") 
      (dolist (file (dired-get-marked-files nil arg)) 
        (if file (movefile file)) 
        (movefile buffer-file-name))))

(global-set-key (kbd "C-c u m") 'movefile)
...
(add-hook 'window-configuration-change-hook 
          (lambda () 
            (progn 
              (setq org-tags-column (- (window-body-width) 20))) 
            (org-align-all-tags)))
...
;;   :config
;;       (setq-default org-download-image-dir "~/Dropbox/notePicture"))
